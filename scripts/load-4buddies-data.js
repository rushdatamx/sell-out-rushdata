...
// IDs fijos para 4BUDDIES
const TENANT_ID = 'c529e450-3708-48c9-90ca-cfb2a01a57ed'; // 4BUDDIES
const RETAILER_ID = 3; // HEB para 4BUDDIES

// Rutas de archivos
const VENTAS_PATH = '/Users/jmariopgarcia/Desktop/4BUDDIES HEB/HEB SUPABASE/DATA VENTA 4BUDDIES HEB 2024-2025.xlsx';
const INVENTARIO_PATH = '/Users/jmariopgarcia/Desktop/4BUDDIES HEB/HEB SUPABASE/INVENTARIO 30 DÍAS.xlsx';

// Función para convertir fecha serial de Excel a formato YYYY-MM-DD
function excelDateToJSDate(serial) {
  if (typeof serial === 'string') {
    // Si ya es una fecha en string, intentar parsearla
...
async function loadTiendas() {
  ...
  if (!complementosSheet) {
    console.error('❌ No se encontró la hoja COMPLEMENTOS en el archivo de ventas');
    console.log('   Hojas disponibles:', workbook.SheetNames);
    throw new Error('Hoja COMPLEMENTOS no encontrada');
  }
  ...
  if (tiendas.length === 0) {
    console.error('❌ No se encontraron tiendas para insertar');
    throw new Error('No hay tiendas');
  }
...
async function loadProductos() {
  ...
  if (!complementosSheet) {
    console.error('❌ No se encontró la hoja COMPLEMENTOS en el archivo de inventario');
    console.log('   Hojas disponibles:', workbook.SheetNames);
    throw new Error('Hoja COMPLEMENTOS no encontrada');
  }
  ...
  for (let i = 3; i < data.length; i++) {
    ...
    if (!productosMap.has(upc)) {
      productosMap.set(upc, {
        ...
        categoria: 'Snacks', // Categoría genérica para 4BUDDIES
        marca: '4BUDDIES',
        activo: true
      });
    }
...
async function loadVentas(tiendasMap, productosMap) {
  ...
  if (!ventasSheet) {
    console.error('❌ No se encontró la hoja VENTAS');
    console.log('   Hojas disponibles:', workbook.SheetNames);
    throw new Error('Hoja VENTAS no encontrada');
  }
  ...
  if (ventas.length === 0) {
    console.error('❌ No se encontraron ventas para insertar');
...
async function loadInventario(tiendasMap, productosMap) {
  ...
  if (!invSheet) {
    console.error('❌ No se encontró la hoja INV');
    console.log('   Hojas disponibles:', workbook.SheetNames);
    throw new Error('Hoja INV no encontrada');
  }
  ...
  if (inventarios.length === 0) {
    console.error('❌ No se encontraron inventarios para insertar');
    throw new Error('No hay inventarios');
  }
...
async function main() {
  ...
  console.log('='.repeat(80));
  console.log(`Tenant: 4BUDDIES (${TENANT_ID})`);
  console.log(`Retailer: HEB (ID: ${RETAILER_ID})`);
  console.log(`\nArchivos:`);
  console.log(`   Ventas: ${VENTAS_PATH}`);
  console.log(`   Inventario: ${INVENTARIO_PATH}`);
