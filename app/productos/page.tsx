...
export default function ProductosPage() {
  ...
  // Queries
  const { data: filtros } = useProductosFiltros()
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="text-right font-medium">
          {formatCurrency(row.original.ventas)}
        </div>
      ),
    },
    {
      ...
      cell: ({ row }) => (
        <div className="text-right">
          {formatNumber(row.original.unidades)}
        </div>
      ),
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className={`text-right font-medium flex items-center justify-end gap-1 ${
            variacion > 0 ? "text-green-600" : variacion < 0 ? "text-red-500" : ""
          }`}>
            {variacion > 0 ? <TrendingUp className="h-3 w-3" /> : variacion < 0 ? <TrendingDown className="h-3 w-3" /> : null}
  ...
  const kpiCards = [
    {
      ...
      subtitle: "Productos con venta en el período",
    },
    {
      title: "Venta Total",
      value: kpis ? formatCurrency(kpis.venta_total) : "-",
      subtitle: "Ventas del período seleccionado",
    },
    {
