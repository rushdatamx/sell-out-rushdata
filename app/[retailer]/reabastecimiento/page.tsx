...
import { ReabastecimientoMercoTabla } from "@/components/charts/reabastecimiento-merco-tabla"
import { ReabastecimientoMercoAlertasA } from "@/components/charts/reabastecimiento-merco-alertas-a"
import { ReabastecimientoMercoABC } from "@/components/charts/reabastecimiento-merco-abc"
import { ReabastecimientoMercoVentaCero } from "@/components/charts/reabastecimiento-merco-venta-cero"
...
export default function RetailerReabastecimientoPage() {
  ...
  const { data: filtros } = useReabastecimientoFiltros(retailerId)
  ...
  // Check if this is MERCO retailer (id = 4)
  const isMerco = retailer?.id === 4
  ...
  // MERCO-specific layout with monthly snapshot data
  if (isMerco) {
    return (
      <div className="flex-1 space-y-6 p-6 premium-bg gradient-overlay min-h-screen">
        ...
        <div className="sticky-header-blur flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-bold tracking-tight">Reabastecimiento - {retailer?.nombre || ""}</h1>
              <p className="text-muted-foreground mt-1 flex items-center gap-2">
                Cobertura estimada y sugerido de compra
                <span className="w-2 h-2 rounded-full bg-orange-500 pulse-soft" />
              </p>
            </div>
          </div>
        </div>

        {/* Disclaimer for MERCO monthly data */}
        <Alert className="border-orange-200 bg-orange-50/50">
          <Info className="h-4 w-4 text-orange-600" />
          <AlertDescription className="text-orange-800">
            <strong>Nota:</strong> Los días de cobertura son una estimación basada en el snapshot mensual de inventario.
            La venta promedio se calcula con datos del último mes disponible.
          </AlertDescription>
        </Alert>

        {/* MERCO KPIs */}
        <ReabastecimientoMercoKpis retailerId={retailerId} />

        {/* MERCO Charts Grid */}
        <div className="grid gap-6 md:grid-cols-2">
          <ReabastecimientoMercoABC retailerId={retailerId} />
          <ReabastecimientoMercoAlertasA retailerId={retailerId} />
        </div>

        {/* Venta Cero Section */}
        <div className="grid gap-6 md:grid-cols-1">
          <ReabastecimientoMercoVentaCero retailerId={retailerId} />
        </div>

        {/* MERCO Full Table */}
        <ReabastecimientoMercoTabla retailerId={retailerId} />
      </div>
    )
  }

  // HEB/Standard layout (original code)
  return (
    <div className="flex-1 space-y-6 p-6 premium-bg gradient-overlay min-h-screen">
      {/* Header */}
      <div className="sticky-header-blur flex items-center justify-between">
