...
export default function RetailerDashboardPage() {
  ...
  const { data: metrics, isLoading, error } = useDashboardMetrics(Number(diasSeleccionados), retailerId)

  // Generar sparkline con interpolación cuando hay pocos puntos de datos
  const generateSparkline = (data: { value: number }[], minPoints: number = 12) => {
    if (data.length === 0) return [{ value: 0 }, { value: 0 }]
    if (data.length >= minPoints) return data

    // Si hay pocos puntos (ej: datos mensuales), interpolar para crear una curva suave
    const interpolated: { value: number }[] = []
    const pointsNeeded = Math.max(minPoints, data.length * 4)

    for (let i = 0; i < pointsNeeded; i++) {
      const t = i / (pointsNeeded - 1)
      ...
      const value = data[lower].value + (data[upper].value - data[lower].value) * fraction
      // Agregar pequeña variación para que se vea más natural
      const variation = value * 0.02 * Math.sin(i * 0.5)
      interpolated.push({ value: value + variation })
    }

    return interpolated
  }

  const ventasSparkline = generateSparkline(
    metrics?.ventasPorDia.map((d) => ({ value: d.value })) || []
  )
  ...
  return (
    <div className="flex-1 space-y-6 premium-bg gradient-overlay min-h-screen">
      ...
      {/* Ventas Mensuales YoY */}
      <VentasMensualesChart retailerId={retailerId} />

      {/* Top Productos y Top Tiendas */}
      <div className="grid gap-6 md:grid-cols-2">
        <TopProductosChart dias={Number(diasSeleccionados)} retailerId={retailerId} />
        <TopTiendasChart dias={Number(diasSeleccionados)} retailerId={retailerId} />
      </div>

      {/* Mix por Categoría y Estacionalidad */}
      <div className="grid gap-6 md:grid-cols-2">
        <MixCategoriasChart dias={Number(diasSeleccionados)} retailerId={retailerId} />
        <EstacionalidadSemanalWidget dias={Number(diasSeleccionados)} retailerId={retailerId} />
      </div>
